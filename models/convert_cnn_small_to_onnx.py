...
class CNN_Audio(torch.nn.Module):
    def __init__(
        ...
        # Tính toán kích thước đầu ra của lớp convolution một cách động
        # Đảm bảo rằng CNN_Audio được khởi tạo với một dummy_input để tính toán flattened_size và pooled_size
        with torch.no_grad():
...
def convert_and_verify_to_onnx():
    try:
        # Set device
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"Using device: {device}")

        # Initialize configuration for CNN_Small
        base_config = Config()
        base_params = {
        ...
        checkpoint_path = "F:\\Deepfake-Audio-Detector\\models\\.pth\\best_model_CNN_Small_cnn_3s_dataset_102208.pth"
        if not os.path.exists(checkpoint_path):
            raise FileNotFoundError(f"Checkpoint file not found: {checkpoint_path}")

        # Cho phép tải các đối tượng NumPy an toàn
        # Điều này cần thiết nếu checkpoint của bạn chứa các đối tượng numpy.core.multiarray.scalar
        # và bạn đang sử dụng phiên bản PyTorch mới hơn với tính năng bảo mật nâng cao khi tải.
        torch.serialization.add_safe_globals([np._core.multiarray.scalar]) # type: ignore

        print(f"Loading checkpoint from {checkpoint_path}")
        # Tải toàn bộ checkpoint dictionary (chứa model_state_dict và các thông tin khác)
        ...
        # Kích thước Batch Size = 1, Channels = config.in_channels, Height = config.img_size, Width = config.img_size
        dummy_input = torch.randn(
            1, config.in_channels, config.img_size, config.img_size
        ).to(device)

        # Define output path
        output_onnx_path = "F:\\Deepfake-Audio-Detector\\models\\.onnx\\CNN_Small.onnx"
        ...
        torch.onnx.export(
            ...
            output_onnx_path,
            export_params=True,  # Xuất trọng số vào tệp ONNX
            opset_version=11,  # Phiên bản ONNX opset. 11 là một lựa chọn tốt và tương thích.
            do_constant_folding=True,  # Tối ưu hóa biểu đồ ONNX bằng cách tính toán trước các hằng số.
            input_names=["input"],  # Đặt tên cho đầu vào
            output_names=["output"],  # Đặt tên cho đầu ra
            dynamic_axes={  # Cho phép kích thước lô (batch size) thay đổi
                "input": {0: "batch_size"},
                "output": {0: "batch_size"},
            },
        )
        print(f"Successfully converted CNN_Small to ONNX format at {output_onnx_path}")
        ...
        # Chọn provider. 'CPUExecutionProvider' luôn có sẵn. 'CUDAExecutionProvider' nếu bạn có GPU và đã cài đặt ONNX Runtime GPU.
        try:
            sess_options = onnxruntime.SessionOptions()
            # Tùy chọn để in log chi tiết hơn nếu có lỗi
            # sess_options.log_severity_level = 0 # 0 for verbose, 1 for info, 2 for warning (default), 3 for error
        ...
        # Lấy tên đầu vào từ mô hình ONNX
        onnx_input_name = sess.get_inputs()[0].name
        onnx_output_name = sess.get_outputs()[0].name

        # Chuyển dummy_input sang định dạng numpy để ONNX Runtime sử dụng
        onnx_input = {onnx_input_name: dummy_input.cpu().numpy()}
        onnx_output = sess.run([onnx_output_name], onnx_input)[0]

        # So sánh kết quả
        # rtol (relative tolerance): độ lệch tương đối cho các giá trị lớn
        # atol (absolute tolerance): độ lệch tuyệt đối cho các giá trị nhỏ
        np.testing.assert_allclose(pytorch_output, onnx_output, rtol=1e-05, atol=1e-06)
        print("Verification successful: PyTorch and ONNX outputs match!")
        print(f"Sample PyTorch output:\n{pytorch_output[0, :5]}")
        print(f"Sample ONNX output:\n{onnx_output[0, :5]}")
